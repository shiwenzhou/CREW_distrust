...
from crew_algorithms.sac.imitation_learning import ImitationLearningWrapper
from crew_algorithms.sac.policy import (
    ContinuousActorNet,
    ContinuousQValueNet,
    FeedbackNet,
)
...
def make_env(
    ...
    env = TransformedEnv(
        make_base_env(cfg, device, written_feedback_channel=written_feedback_channel, use_written_feedback=written_feedback),
        Compose(
...
def make_agent_continuous(proof_env, cfg, device):
    ...
    actor_extractor = TensorDictModule(
        actor_extractor, in_keys=["actor_out"], out_keys=["loc", "scale"]
    )
...
def make_loss_module(cfg, env, model):
    ...
    if isinstance(env.action_spec.space, ContinuousBox):
        ...
        print("Does not support discrete SAC")
        exit(0)
