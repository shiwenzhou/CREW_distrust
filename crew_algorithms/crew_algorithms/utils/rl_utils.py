...
def make_base_env(
    ...
    timing_args = []
    if hasattr(env_cfg, "steps_per_decision"):
    timing_args += ["-DecisionRequestFrequency", f"{env_cfg.steps_per_decision}"]
    if hasattr(env_cfg, "max_steps"):
    timing_args += ["-MaxSteps", f"{env_cfg.max_steps}"]

    quadrant_experiment_args = []
    if hasattr(env_cfg, "quadrant_distrust_experiment"):
    	quadrant_experiment_args += [
        "-QuadrantDistrustExperiment",
        "1" if env_cfg.quadrant_distrust_experiment else "0",
        "-MalfunctionAfterSections",
        f"{env_cfg.malfunction_after_sections}",
        "-MalfunctionTimeoutSteps",
        f"{env_cfg.malfunction_timeout_steps}",
        "-SectionCompletionGraceSteps",
        f"{env_cfg.section_completion_grace_steps}",
        "-SupportOverrideSteps",
        f"{env_cfg.support_override_steps}",
    ]
    game_args = []
